...
async function recoverPublicKey({ hash: hash2, signature }) {
  ...
  const { secp256k1: secp256k12 } = await import("./secp256k1-WIV63BTH.js");
  const signature_ = (() => {
...
function getCreateAddress(opts) {
  const from12 = toBytes(getAddress(opts.from));
  let nonce = toBytes(opts.nonce);
  if (nonce[0] === 0)
    nonce = new Uint8Array([]);
  return getAddress(`0x${keccak256(toRlp([from12, nonce], "bytes")).slice(26)}`);
}
function getCreate2Address(opts) {
  ...
  return getAddress(slice(keccak256(concat([toBytes("0xff"), from12, salt, bytecodeHash])), 12));
}
...
function ccipReadTunnel({ batchGateways, ccipRequest: ccipRequest2 = ccipRequest }) {
  return {
...
//# sourceMappingURL=chunk-OZVRAXMV.js.map
