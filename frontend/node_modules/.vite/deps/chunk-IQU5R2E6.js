...
function assertEip712Transaction(transaction) {
  const { chainId, to, from: from21, paymaster, paymasterInput } = transaction;
  ...
  if (from21 && !isAddress(from21))
    throw new InvalidAddressError({ address: from21 });
...
function serializeTransactionEIP712(transaction) {
  const { chainId, gas, nonce: nonce2, to, from: from21, value, maxFeePerGas, maxPriorityFeePerGas, customSignature, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
...
function transactionToMessage(transaction) {
  const { gas, nonce: nonce2, to, from: from21, value, maxFeePerGas, maxPriorityFeePerGas, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
  return {
    txType: 113n,
    from: BigInt(from21),
    to: to ? BigInt(to) : 0n,
...
function serializeTransactionDeposit(transaction) {
  ...
  const { sourceHash, data, from: from21, gas, isSystemTx, mint: mint2, to, value } = transaction;
  const serializedTransaction = [
    sourceHash,
    from21,
    to ?? "0x",
...
function assertTransactionDeposit(transaction) {
  const { from: from21, to } = transaction;
  if (from21 && !isAddress(from21))
    throw new InvalidAddressError({ address: from21 });
  if (to && !isAddress(to))
    throw new InvalidAddressError({ address: to });
...
__export(SignatureEnvelope_exports, {
  ...
  serialize: () => serialize,
  sortMultisigApprovals: () => sortMultisigApprovals,
  toRpc: () => toRpc3,
  types: () => types,
  validate: () => validate5,
  verify: () => verify4
...
// node_modules/ox/_esm/tempo/MultisigConfig.js
var MultisigConfig_exports = {};
__export(MultisigConfig_exports, {
  ...
  maxOwnerSignatureBytes: () => maxOwnerSignatureBytes,
  maxOwners: () => maxOwners,
  signatureTypeByte: () => signatureTypeByte,
  toId: () => toId,
  toTuple: () => toTuple2,
  validate: () => validate4,
  zeroSalt: () => zeroSalt
...
var maxOwners = 10;
var maxOwnerSignatureBytes = 2049;
var signatureTypeByte = "0x05";
var zeroSalt = `0x${"00".repeat(32)}`;
...
function assert4(config) {
  const { salt, threshold, owners } = config;
  if (typeof salt !== "undefined" && size(salt) !== 32)
  ...
  for (const owner of owners) {
    if (!validate3(owner.owner) || toBigInt2(owner.owner) === 0n)
    ...
    const current = toBigInt2(owner.owner);
...
function from8(config) {
  const owners = [...config.owners].sort((a, b) => toBigInt2(a.owner) < toBigInt2(b.owner) ? -1 : 1);
  const normalized = {
    salt: config.salt ? padLeft2(config.salt, 32) : zeroSalt,
...
function fromTuple2(tuple) {
  ...
  return {
    salt: salt && salt !== "0x" ? padLeft2(salt, 32) : zeroSalt,
...
function getAddress(value) {
  ...
  const hash5 = keccak2562(concat2(fromString2(accountDomain), id));
  return from4(slice2(hash5, 12, 32));
}
function getSignPayload(value) {
  ...
  return keccak2562(concat2(fromString2(signatureDomain), from2(payload), account, genesisConfigId));
}
function toId(config) {
  assert4(config);
  const id = keccak2562(concat2(fromString2(configDomain), padLeft2(config.salt ?? zeroSalt, 32), fromNumber2(config.threshold, { size: 4 }), fromNumber2(config.owners.length, { size: 4 }), ...config.owners.flatMap((owner) => [
    owner.owner,
    fromNumber2(owner.weight, { size: 4 })
  ])));
  if (toBigInt2(id) === 0n)
    throw new InvalidConfigError({ reason: "config ID cannot be zero" });
  return id;
}
function toTuple2(config) {
  assert4(config);
  const owners = config.owners.map((owner) => [owner.owner, fromNumber2(owner.weight)]);
  const salt = config.salt ? padLeft2(config.salt, 32) : zeroSalt;
  return [salt, fromNumber2(config.threshold), owners];
}
function validate4(config) {
  try {
    assert4(config);
    return true;
  } catch {
...
var InvalidConfigError = class extends BaseError2 {
  constructor({ reason }) {
...
var serializedMultisigType = "0x05";
...
function deserialize(value) {
  ...
  if (typeId === serializedMultisigType) {
    const [account, genesisConfigId, signatures, init] = toHex2(data);
    return {
      type: "multisig",
      account,
      genesisConfigId,
...
function from9(value, options) {
  ...
  if (type2 === "multisig") {
    ...
    return {
      ...
      account,
      genesisConfigId,
      signatures: rest.signatures.map((signature) => from9(signature)),
      ...
      ...initSource ? { init: from8(initSource) } : {},
      type: type2
    };
  }
  return {
...
function fromRpc3(envelope) {
  ...
  if (envelope.type === "keychain" || "userAddress" in envelope && "signature" in envelope) {
    ...
    return {
      ...
      inner: fromRpc3(keychain.signature),
  ...
  if (envelope.type === "multisig" || "account" in envelope && "configId" in envelope && "signatures" in envelope) {
    ...
    return {
      ...
      ...multisig.init ? { init: from8(multisig.init) } : {}
    };
  }
...
function serialize(envelope, options = {}) {
  ...
  if (type2 === "multisig") {
    const multisig = envelope;
    return concat2(serializedMultisigType, fromHex3([
      multisig.account,
      multisig.genesisConfigId,
      multisig.signatures.map((signature) => serialize(signature)),
      multisig.init ? toTuple2(multisig.init) : "0x"
...
function sortMultisigApprovals(value) {
  ...
  const digest = getSignPayload("genesisConfig" in value && value.genesisConfig ? { payload, genesisConfig: value.genesisConfig } : {
    payload,
    account: value.account,
    genesisConfigId: value.genesisConfigId
  });
  return signatures.map((signature) => ({
    key: toBigInt2(extractAddress({ payload: digest, signature })),
    signature
...
function verify4(signature, parameters2) {
  const { payload } = parameters2;
  const address2 = (() => {
    if (parameters2.address)
      return parameters2.address;
    if (parameters2.publicKey)
      return fromPublicKey(parameters2.publicKey);
    return void 0;
  })();
  if (!address2)
    return false;
  const envelope = from9(signature);
  if (envelope.type === "secp256k1") {
    if (!address2)
      return false;
    return verify({
...
function validate6(tempoAddress) {
  try {
    parse(tempoAddress);
    return true;
  } catch {
...
function hash(authorization, options = {}) {
  ...
  return keccak2562(concat2("0x05", fromHex3(toTuple3(presign ? {
...
function from11(tokenIdOrAddress) {
...
var eip712DomainTypehash = keccak2562(fromString2("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)"));
var nameHash = keccak2562(fromString2("TIP20 Channel Reserve"));
var versionHash = keccak2562(fromString2("1"));
var zeroAddress = "0x0000000000000000000000000000000000000000";
var address = "0x4d50500000000000000000000000000000000000";
var closeGracePeriod = 900n;
var voucherTypehash = keccak2562(fromString2("Voucher(bytes32 channelId,uint96 cumulativeAmount)"));
...
__export(KeyAuthorization_exports, {
  InvalidAdminMarkerError: () => InvalidAdminMarkerError,
  InvalidWitnessSizeError: () => InvalidWitnessSizeError,
  deserialize: () => deserialize2,
  from: () => from13,
  fromRpc: () => fromRpc5,
  fromTuple: () => fromTuple4,
...
function from13(authorization, options = {}) {
  ...
  if (options.signature)
    return {
      ...resolved,
      signature: from9(options.signature)
    };
  return resolved;
}
function fromRpc5(authorization) {
  ...
  return {
    ...
    ...isAdmin2 ? { isAdmin: true } : {},
    ...account !== void 0 ? { account } : {}
  };
}
function fromTuple4(tuple) {
  ...
  const expiry = isAbsent(rawExpiry) ? void 0 : hexToNumber2(rawExpiry) || void 0;
  const limits = Array.isArray(rawLimits) && rawLimits.length > 0 ? rawLimits.map((limitTuple) => {
    ...
    return {
      ...
      ...period !== void 0 ? { period: hexToNumber2(period) } : {}
    };
  }) : void 0;
...
function toRpc5(authorization) {
  const { address: address2, scopes, chainId, expiry, limits, type: type2, signature, witness, isAdmin: isAdmin2, account } = authorization;
  if (witness !== void 0)
    assertWitness(witness);
  const allowedCalls = (() => {
    if (!scopes)
...
function toTuple4(authorization) {
  ...
  const hasTip1053Plus = witness !== void 0 || isAdmin2 || account !== void 0;
  const optionals = [
    {
      value: expiry !== null && expiry !== void 0 && expiry !== 0 ? numberToHex2(expiry) : void 0,
      placeholder: "0x"
    },
    {
      value: limitsValue,
  ...
  const authorizationTuple = [bigintToHex(chainId), type2, address2, ...trailing];
...
var InvalidWitnessSizeError = class extends Error {
  constructor(witness) {
    super(`Witness \`${witness}\` must be exactly 32 bytes (got ${size(witness)} bytes).`);
    Object.defineProperty(this, "name", {
      enumerable: true,
      configurable: true,
      writable: true,
...
var InvalidAdminMarkerError = class extends Error {
  constructor(marker) {
...
var ReceivePolicyReceipt_exports = {};
__export(ReceivePolicyReceipt_exports, {
  decode: () => decode3,
  encode: () => encode3,
  from: () => from16,
  fromLog: () => fromLog,
  fromTransactionReceipt: () => fromTransactionReceipt
});
...
function decode2(...parameters2) {
  const [abiEvent, log] = (() => {
    if (Array.isArray(parameters2[0])) {
      const [abi, name, log2] = parameters2;
...
function encode2(...parameters2) {
  const [abiEvent, args] = (() => {
    if (Array.isArray(parameters2[0])) {
      const [abi, name, args3] = parameters2;
      return [fromAbi2(abi, name), args3];
    }
    const [abiEvent2, args2] = parameters2;
    ...
    if (args_.length > 0) {
      ...
      topics = (indexedInputs == null ? void 0 : indexedInputs.map((param, i) => {
        if (Array.isArray(args_[i]))
          return args_[i].map((_, j) => encode4(param, args_[i][j]));
        return typeof args_[i] !== "undefined" && args_[i] !== null ? encode4(param, args_[i]) : null;
      })) ?? [];
    }
  }
  const selector = (() => {
...
var DataMismatchError = class extends BaseError2 {
  constructor({ abiEvent, data, parameters: parameters2, size: size2 }) {
    super([
      `Data size of ${size2} bytes is too small for non-indexed event parameters.`
    ].join("\n"), {
      metaMessages: [
        `Non-indexed Parameters: (${format(parameters2)})`,
...
// node_modules/ox/_esm/tempo/ReceivePolicyReceipt.js
var blockedReasons = ["none", "tokenFilter", "receivePolicy"];
var kinds = ["transfer", "mint"];
var parameters = [
  {
    type: "tuple",
    components: [
...
var transferBlocked = from15("event TransferBlocked(address indexed token, address indexed receiver, uint64 indexed blockedNonce, uint256 amount, uint8 receiptVersion, bytes receipt)");
function decode3(receipt) {
  const [decoded] = decode(parameters, receipt);
  return {
...
function fromLog(log) {
  const { receipt } = decode2(transferBlocked, log);
  return receipt;
}
function fromTransactionReceipt(receipt) {
  const selector = getSelector2(transferBlocked);
  const receipts = [];
...
function deserialize3(serialized) {
  ...
  if (keyAuthorization)
    transaction.keyAuthorization = fromTuple4(keyAuthorization);
...
function from18(envelope, options = {}) {
  ...
  return {
    ...envelope_,
    ...signature ? { signature: from9(signature) } : {},
...
function serialize4(envelope, options = {}) {
  ...
  const serialized = [
    ...
    ...signature ? [serialize(from9(signature))] : []
  ];
...
function getSignPayload4(envelope, options = {}) {
...
async function resolveNode() {
  ...
  const { wasmBase64 } = await import("./mine.wasm-5V73L2MV.js");
...
async function mineSaltAsync(parameters2) {
  const { chunkSize = 1e5, count = 2 ** 32, onProgress, signal, start: start_ = 0n, workers = getDefaultWorkerCount() } = parameters2;
  const address2 = resolveAddress3(parameters2.address);
...
var fieldsSize = 29;
var version = 0;
function from20(authentication, options = {}) {
...
function getSignPayload5(authentication, options = {}) {
...
function serialize5(authentication, options = {}) {
  ...
  return concat2(serialize(from9(signature)), getFields(authentication));
}
...
async function serializeTempo(transaction, sig) {
  ...
  if (transaction.multisig && transaction.signatures && !feePayer) {
    const payload = TxEnvelopeTempo_exports.getSignPayload(TxEnvelopeTempo_exports.from(transaction_ox));
    const signatures = transaction.signatures.map((approval) => SignatureEnvelope_exports.from(approval));
    const sorted = SignatureEnvelope_exports.sortMultisigApprovals({
      payload,
      signatures,
      genesisConfig: transaction.multisig
    });
    const signature2 = SignatureEnvelope_exports.from({
      genesisConfig: transaction.multisig,
      signatures: sorted,
      ...nonce2 ? {} : { init: true }
    });
...
async function signKeyAuthorization(account, parameters2) {
  const { chainId, key, expiry, limits, scopes, witness, admin } = parameters2;
  ...
  const signature = isAccessKey ? await account.sign({ hash: hash5, raw: true }) : await account.sign({ hash: hash5 });
...
async function authorize(client, parameters2) {
  return authorize.inner(sendTransaction, client, parameters2);
}
(function(authorize2) {
  async function inner(action, client, parameters2) {
    var _a2;
    const { accessKey, admin, chainId = (_a2 = client.chain) == null ? void 0 : _a2.id, expiry, limits, scopes, witness, ...rest } = parameters2;
...
async function burnWitness(client, parameters2) {
  return burnWitness.inner(writeContract, client, parameters2);
}
(function(burnWitness2) {
  async function inner(action, client, parameters2) {
    const { witness, ...rest } = parameters2;
    const call2 = burnWitness2.call({ witness });
    return await action(client, {
      ...rest,
      ...call2
    });
  }
  burnWitness2.inner = inner;
  function call(args) {
    const { witness } = args;
    return defineCall({
  ...
  burnWitness2.extractEvent = extractEvent;
})(burnWitness || (burnWitness = {}));
async function getMetadata(client, parameters2) {
  const { account: account_ = client.account, accessKey, ...rest } = parameters2;
  if (!account_)
    throw new Error("account is required.");
  const account = parseAccount(account_);
...
async function getRemainingLimit(client, parameters2) {
  var _a2;
  const { account: account_ = client.account, accessKey, token, ...rest } = parameters2;
  if (!account_)
    throw new Error("account is required.");
  const account = parseAccount(account_);
...
async function isAdmin(client, parameters2) {
  const { account: account_ = client.account, accessKey, ...rest } = parameters2;
  if (!account_)
    throw new Error("account is required.");
  const account = parseAccount(account_);
...
async function isWitnessBurned(client, parameters2) {
  const { account: account_ = client.account, witness, ...rest } = parameters2;
  if (!account_)
    throw new Error("account is required.");
  const account = parseAccount(account_);
  return readContract(client, {
...
(function(isWitnessBurned2) {
  ...
  isWitnessBurned2.call = call;
})(isWitnessBurned || (isWitnessBurned = {}));
async function revoke(client, parameters2) {
  return revoke.inner(writeContract, client, parameters2);
}
(function(revoke2) {
  async function inner(action, client, parameters2) {
    const { accessKey, ...rest } = parameters2;
...
async function updateLimit(client, parameters2) {
  return updateLimit.inner(writeContract, client, parameters2);
}
(function(updateLimit2) {
  async function inner(action, client, parameters2) {
    const { accessKey, token, limit, ...rest } = parameters2;
...
async function verifyHash2(client, parameters2) {
  const { account, admin, hash: hash5, signature, ...rest } = parameters2;
  return readContract(client, {
    ...rest,
    ...verifyHash2.call({ account, admin, hash: hash5, signature })
  });
}
(function(verifyHash3) {
  function call(args) {
    const { account, admin = true, hash: hash5, signature } = args;
    return defineCall({
      address: signatureVerifier2,
      abi: signatureVerifier,
      functionName: admin ? "verifyKeychainAdmin" : "verifyKeychain",
      args: [account, hash5, signature]
    });
  }
  verifyHash3.call = call;
...
var chainConfig5 = {
  ...
  prepareTransactionRequest: [
    async (r, { client, phase }) => {
      ...
      const multisig = request.multisig ?? (((_c = request.account) == null ? void 0 : _c.source) === "multisig" ? request.account.config : void 0);
      if (multisig) {
        request.multisig = multisig;
        request.from = MultisigConfig_exports.getAddress(multisig);
        if (((_d = request.account) == null ? void 0 : _d.source) !== "multisig")
          delete request.account;
      }
      if (!request.keyAuthorization && ((_e = request.account) == null ? void 0 : _e.source) === "accessKey") {
        const keyAuthorizationManager = request.account.keyAuthorizationManager;
        if (keyAuthorizationManager) {
          const chainId = request.chainId ?? ((_f = request.chain) == null ? void 0 : _f.id);
      ...
      if (!request.feeToken && ((_g = request.chain) == null ? void 0 : _g.feeToken))
  ...
  async verifyHash(client, parameters2) {
    const { address: address2, hash: hash5, signature, mode: mode2 } = parameters2;
    const envelope = (() => {
    ...
    if (envelope) {
      if ((envelope == null ? void 0 : envelope.type) === "keychain" && mode2 === "allowAccessKey") {
        ...
        const keyInfo = await getMetadata(client, {
          ...
          blockNumber: parameters2.blockNumber,
          blockTag: parameters2.blockTag
        });
    ...
    return await getAction(client, verifyHash, "verifyHash")({ ...parameters2, chain: null });
  }
};
...
var valygoSmartchain = defineChain({
  id: 7771777,
...
// node_modules/viem/_esm/chains/definitions/whitechainSepolia.js
var whitechainSepolia = defineChain({
  testnet: true,
  id: 1874,
...
__export(chains_exports, {
  ...
  unreal: () => unreal,
  valygoNft: () => valygoNft,
  valygoSmartchain: () => valygoSmartchain,
  vana: () => vana,
  ...
  whitechainSepolia: () => whitechainSepolia,
...
//# sourceMappingURL=chunk-IQU5R2E6.js.map
